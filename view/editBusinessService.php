...
require_once("../controller/BusinessServiceController.php");
session_start();
?>
<!DOCTYPE html>
<html lang="en">
...
<header>
    ...
    <main class="retract d-flex flex-column align-items-center">
        <h2>Edit Service</h2>
        <form action="../controller/EditServiceController.php" method="post" class="d-flex flex-column w-50 gap-2 mb-2">
            <input type="hidden" name="id" value="<?php echo $specificService->id; ?>">

            <label for="categoryId">Category:</label>
            <input type="text" id="categoryId" name="categoryId" value="<?php echo $specificService->categoryId; ?>" required>

            <label for="name">Name:</label>
            <input type="text" id="name" name="name" value="<?php echo $specificService->name; ?>" required>

            <label for="price">Price:</label>
            <input type="number" step="0.01" id="price" name="price" value="<?php echo $specificService->price; ?>" required>

            <label for="description">Description:</label>
            <textarea id="description" name="description" required><?php echo $specificService->description; ?></textarea>
